...
merged_obj <- merge(x=AR_G1,y=c(AR_GFP,Sham),add.cell.ids = c("AR_G1","AR_GFP","Sham"),project = "Maternal")
...
pdf("./01_QC/AR_G1_normalization_plot.pdf")
...
plot2
dev.off();
pdf("./01_QC/AR_GFP_normalization_plot.pdf")
Maternal.list[[2]] <- NormalizeData(Maternal.list[[2]],verbose = FALSE)
Maternal.list[[2]] <- FindVariableFeatures(Maternal.list[[2]],selection.method = "vst", nfeatures = 2000, verbose = FALSE)
top10 <- head(VariableFeatures(Maternal.list[[2]]),10)
plot1 <- VariableFeaturePlot(Maternal.list[[2]])
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1
plot2
dev.off();
pdf("./01_QC/Sham_normalization_plot.pdf")
Maternal.list[[3]] <- NormalizeData(Maternal.list[[3]],verbose = FALSE)
Maternal.list[[3]] <- FindVariableFeatures(Maternal.list[[3]],selection.method = "vst", nfeatures = 2000, verbose = FALSE)
top10 <- head(VariableFeatures(Maternal.list[[3]]),10)
plot1 <- VariableFeaturePlot(Maternal.list[[3]])
plot2 <- LabelPoints(plot = plot1, points = top10, repel = TRUE)
plot1
plot2
dev.off();
...
pdf("./02_G1/01_All_celltype/seclect_pca.pdf")
...
Maternal.integrated <- FindClusters(object = Maternal.integrated, resolution = 1.2)
Maternal.integrated <- RunUMAP(object = Maternal.integrated, dims = 1:25)

table(Maternal.integrated@active.ident)

pdf("./02_G1/01_All_celltype/Unsupervised_cluster_Umap.pdf")
...
saveRDS(Maternal.integrated, file = "./02_G1/01_All_celltype/Maternal_integrated_G1.rds")
